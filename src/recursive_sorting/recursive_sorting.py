...
def merge(arrA, arrB):
    ...
    while len(arrA) > 0 and len(arrB) > 0:
        #compare the first elements of each array, already sorted
        # If the first element from arrA is lower or equal to the first element of arrB, append this value to our merged array
        ...
        #do the same (but opposite) for arrB
            ...
            arrB = arrB[1:]
    #exit the while loop when either arrA or arrB is empty. Add their contents onto the merged array
...
def merge_in_place(arr, start, mid, end):
    ...
    while (start <= mid and right_start <= end):
        ...
        #check if the lowest value from the left side is less than or equal to the lowest from the right
        #  If yes, the value is already in the correct position
        # we need to move the start pointer to move on to the next comparison
        if arr[start] <= arr[right_start]:
            #its already in the right place, so increment start
            start +=1
        #otherwise, the lowest element in the right half is greater than the lowest element in the left half
        #  In this case we need to move this to the start and shift everything else over by inserting that value where start was, and then deleting the element where it existed before (plus 1 to account for everything shifting over 1 index)
        else:
...
def quick_sort(data):
    ...
    if len(data) == 0:
        return data
    # partition handles picking the pivot and partioning data around pivot
    # left is left sublist, right is right sublist, pivot is solo
